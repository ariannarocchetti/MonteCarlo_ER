...
command = ("ls -d raw_files/%s*" %localfile)
...
for i in range(5, len(out)-1):
     #os.system("rsync -r %s:%s/%s %s" % (scp_path, mc_dir,out[i],storage_path) )
...
##########hadd nSorted files 
        ...
        os.system("hadd -f -k %s %s"%(name_final_file, name_final_file))
"""
